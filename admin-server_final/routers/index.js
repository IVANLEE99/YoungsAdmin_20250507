...
// https://free-api.com/doc/253
router.get("/ipAreaCoordJson", async (req, res) => {
  try {
    const coords = req.query.coords || "116.415767,39.91095"; // 默认北京地址
    const apiUrl = `http://whois.pconline.com.cn/ipAreaCoordJson.jsp?json=true&coords=${coords}`;

    // 调用第三方天气API
    const response = await axios.get(apiUrl, {
      responseType: "arraybuffer",
    });
    // 检测实际编码（部分网站可能实际用gb2312）
    const isGB2312 = response.headers["content-type"].includes("gb2312");
    const encoding = isGB2312 ? "gb2312" : "gbk";
...
// https://note.youdao.com/ynoteshare/index.html?id=6a7261fca117d7b3ca8fc08de720b00c&type=note&_time=1747540263397
router.get("/weather", async (req, res) => {
  try {
    const cityId = req.query.cityId || "101010100"; // 默认北京市ID
